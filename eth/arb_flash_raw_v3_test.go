...
//go:embed arb/testdata/node-evm-fixture.json
var flashRawFixtureJSON []byte

type flashLegV3 struct {
	AdapterId         uint16
...
func TestExecutorV3ActualSignedFlashBundle(t *testing.T) {
	var fixture struct {
		Init string          `json:"fixture_init_code"`
		Auth string          `json:"auth_runtime_code"`
		ABI  json.RawMessage `json:"executor_abi"`
	}
	if err := json.Unmarshal(flashRawFixtureJSON, &fixture); err != nil {
		t.Fatal(err)
	}
	...
	leg := func(pool, in, out common.Address) flashLegV3 {
		return flashLegV3{AdapterId: 1, PoolOrManager: pool, TokenIn: in, TokenOut: out, MinAmountOut: new(big.Int), SqrtPriceLimitX96: new(big.Int), AdapterData: fees}
	}
	plan := flashPlanV3{OpportunityId: common.HexToHash("0x01"), TargetTxHash: target.Hash(), RequiredParentHash: parent.Hash(), TargetBlockNumber: parent.Number.Uint64() + 1, BaseToken: wbnb, AmountIn: coin(1), MinRetainedBeforeGas: big.NewInt(10000000000000000), MaxGasPriceWei: price, BuilderPaymentRecipient: recipient, BuilderPaymentWei: big.NewInt(10000000000000000), GasLimit: big.NewInt(1400000), Legs: []flashLegV3{leg(p1, wbnb, token), leg(p2, token, wbnb)}}
